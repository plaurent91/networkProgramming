...
public class Application implements Runnable {
	...
	public final static int TakingPhoto = 5;

	// Other constants
	public final static String statusMessages[] = {
		" Error! Could not connect!", " Disconnected",
		" Disconnecting...", " Connecting...", " Connected","Taking Photo"
	};
	public final static Application tcpObj = new Application();
	public final static String END_CHAT_SESSION =
			"End Session"; // Indicates the end of a session
	...
	public static JButton takePhoto = null;
	...
	private static JPanel initOptionsPane() {
		...
		buttonListener = new ActionAdapter() {
			public void actionPerformed(ActionEvent e) {
				...
				//take photo
				if (e.getActionCommand().equals("takePic")) {
					changeStatusNTS(TakingPhoto, true);
				}
		...
		takePhoto.setMnemonic(KeyEvent.VK_T);
		takePhoto.setActionCommand("takePic");
		takePhoto.addActionListener(buttonListener);
	...
	public static void initGUI() {
		...
		mainFrame = new JFrame("SURF Visualization");
	...
	public void run() {
		...
		case CONNECTED:
			...
			if (!isHost) {
				...
				if (chatText.getText().length() == 0)
					chatText.setText("Type help for command buttons\n\n");
					mainFrame.setTitle("GUEST");}
			else {
				chatText.setText("Host is connected");
				chatText.setEnabled(false);
				mainFrame.setTitle("HOST");
			}
			ipField.setEnabled(false);
			portField.setEnabled(false);
			hostOption.setEnabled(false);
		...
		case BEGIN_CONNECT:
			...
			statusColor.setBackground(Color.orange);
			if (isHost)
				chatText.setText("Waiting for host to connect!");
			break;
	...
	public void startGUI() {
		...
		SendData sendData = new SendData(tcpObj);
		...
		while (true) {
			...
			case BEGIN_CONNECT:
				try {
					sendData.setUpServer();
					in = new BufferedReader(new 
							InputStreamReader(socket.getInputStream()));
					out = new PrintWriter(socket.getOutputStream(), true);
					sendData = new SendData (in, out, tcpObj);
					receiveData = new ReceiveData(in, out, tcpObj);
			...
			case CONNECTED:
				try {
					// Send data
					if (toSend.length() != 0) {
						sendData.sendData(toSend);
						changeStatusTS(NULL, true);
					}

					// Receive data
					if (in.ready()) 
						receiveData.analyzeData(in.readLine());
				}
				catch (IOException e) {
					cleanUp();
					changeStatusTS(DISCONNECTED, false);
